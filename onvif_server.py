...
<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope"
    ...
    <soap:Header>
        ...
        {f'<wsa:Action xmlns:wsa="http://www.w3.org/2005/08/addressing">{action}</wsa:Action>' if action else ''}
    </soap:Header>
    <soap:Body>
        {body_content}
    </soap:Body>
...
async def device_service(request: Request, user: str = Depends(verify_credentials)):
    """Handle Device Management Service requests"""
    xml_content = await request.body()
    soap_request = parse_soap_request(xml_content.decode('utf-8'))
...
async def media_service(request: Request, user: str = Depends(verify_credentials)):
...
async def ptz_service(request: Request, user: str = Depends(verify_credentials)):
